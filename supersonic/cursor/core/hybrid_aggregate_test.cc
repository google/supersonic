...
TEST_F(HybridAggregateTest, CountAllColumnDistinct) {
  ...
  scoped_ptr<Cursor> aggregate(
      SucceedOrDie(CreateGroupAggregate(empty_projector_, aggregation, input)));
  scoped_ptr<Cursor> expected_output(
      TestDataBuilder<UINT64, UINT64, UINT64>()
      .AddRow(3, 2, 1)
      .BuildCursor());
  EXPECT_FALSE(aggregate->schema().attribute(0).is_nullable());
...
TEST_F(HybridAggregateTest, GroupByTwoColumns) {
  ...
  scoped_ptr<Cursor> aggregate(
      SucceedOrDie(CreateGroupAggregate(*group_by_columns, aggregation,
                                        input)));
...
TEST_F(HybridAggregateTest, HybridGroupTransformTest) {
  ...
  scoped_ptr<Cursor> transformed(
      SucceedOrDie(BoundHybridGroupAggregate(
          group_by_columns.release(), aggregation,
          "",
          HeapBufferAllocator::Get(), 0,
