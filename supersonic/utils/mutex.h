...
#ifndef SUPERSONIC_OPENSOURCE_AUXILIARY_MUTEX_H_
#define SUPERSONIC_OPENSOURCE_AUXILIARY_MUTEX_H_

// The HAVE_CONFIG_H flag will be set during installation using autotools.
// In this case the required config file resides in the project directory.
// When building an external project which uses Supersonic the config
// file should be sought after in standard installation directories
// (like /usr/local/include for example).
#ifdef HAVE_CONFIG_H
...
#elif defined(SUPERSONIC_HAVE_PTHREAD) && defined(SUPERSONIC_HAVE_RWLOCK)
...
#elif defined(SUPERSONIC_HAVE_PTHREAD)
# include <pthread.h>
...
#include "supersonic/utils/once.h"
#include "supersonic/utils/macros.h"
#include "supersonic/utils/thread_annotations.h"
...
// MutexLockMaybe is like MutexLock, but is a no-op when mu is 0.
...
#define MutexLockMaybe(x) COMPILE_ASSERT(0, mutex_lock_decl_missing_var_name)

#endif  // SUPERSONIC_OPENSOURCE_AUXILIARY_MUTEX_H_
