...
inline uint32 KeyToUint32(StringPiece key) {
  uint32 value;
...
inline uint64 KeyToUint64(StringPiece key) {
  uint64 value;
  DCHECK_EQ(key.size(), sizeof(value));
  memcpy(&value, key.data(), sizeof(value));
  return ntohll(value);
...
inline uint128 KeyToUint128(StringPiece key) {
  uint64 v0, v1;
...
template <typename T> inline bool DecodePOD(StringPiece str, T* result) {
  ENFORCE_POD(T);
  CHECK(result != NULL);
...
inline bool DecodeDouble(StringPiece s, double* d) {
  return DecodePOD(s, d);
}

inline bool DecodeFloat(StringPiece s, float* f) {
  return DecodePOD(s, f);
}

inline bool DecodeUint32(StringPiece s, uint32* i) {
  return DecodePOD(s, i);
}

inline bool DecodeUint64(StringPiece s, uint64* i) {
  return DecodePOD(s, i);
}
...
bool DictionaryInt64Decode(std::unordered_map<string, int64>* dictionary,
                           const string& encoded_str);
