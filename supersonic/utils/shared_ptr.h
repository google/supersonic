...
// of the boost/tr1 shared_ptr class, which is part of C++11 standard
// section 20.7.2.2 [util.smartptr.shared].
//
...
//     shared_ptr for the entire call, and there is a performance benefit.
//
//       void foo(const shared_ptr<T>& param)
//
//  2. Functions that want to provide a shared_ptr to the caller should simply
//     return one, rather than accepting a pointer parameter and setting it.
//     The return is negligibly faster, but more importantly, the code
//     is clearer.
//
//     typedef map<Key, shared_ptr<Value> > MyMap;
//     shared_ptr<Value> GetAndSharedObject(const Key& key) {
//       ReaderMutexLock l(&lock_);
//       MyMap::iterator iter = shared_container.find(key);
//       if (iter == shared_container.end())
...
#ifndef UTIL_GTL_SHARED_PTR_H__
#define UTIL_GTL_SHARED_PTR_H__
...
using std::reverse;
using std::sort;
using std::swap;  // for swap

#include "supersonic/utils/atomic_refcount.h"
...
class shared_ptr {
 ...
 public:
  ...
  // TODO(user): Consider providing this constructor. The draft C++ standard
  ...
  ~shared_ptr() {
    if (ptr_ != NULL) {
      if (!base::RefCountDec(&control_block_->refcount_)) {
        delete ptr_;

        // weak_count_ is defined as the number of weak_ptrs that observe
        // ptr_, plus 1 if refcount_ is nonzero.
        if (!base::RefCountDec(&control_block_->weak_count_)) {
 ...
 private:
  ...
  // NOTE(user): We'd need a more general form to support const_pointer_cast and
  ...
  void InitializeWithStaticCast(const shared_ptr<V>& r) {
    if (r.control_block_ != NULL) {
      base::RefCountInc(&r.control_block_->refcount_);
...
class weak_ptr {
 ...
 public:
  ...
  ~weak_ptr() {
    if (control_block_ != NULL &&
        !base::RefCountDec(&control_block_->weak_count_)) {
      delete control_block_;
    }
  }
 ...
 private:
  void CopyFrom(T* ptr, SharedPtrControlBlock* control_block) {
    ...
    if (control_block_ != NULL)
      base::RefCountInc(&control_block_->weak_count_);
  }
